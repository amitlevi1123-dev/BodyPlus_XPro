// admin_web/static/js/system.js
// -----------------------------------------------------------------------------
// BodyPlus XPro — System Page Updater
// דוגם את /api/system כל שנייה ומעדכן את עמוד system.html
// עמיד לשגיאות: אם יש כשל זמני, משאיר את הערכים הקיימים וממשיך לדגום
// -----------------------------------------------------------------------------
...
(function () {
  ...
  function setBadgeBg(el, percent) {
    ...
    el.style.background =
      percent < 70 ? "#d1fae5" : (percent < 90 ? "#fef3c7" : "#fee2e2");
  }
  ...
  async function pullAndRender() {
    try {
      ...
      if (el.gpuMain) {
        el.gpuMain.textContent = gpuAvail
          ? (gpuP == null ? "N/A" : fmtPercent(gpuP))
          : "אין GPU";
      }
      if (el.gpuSub) {
        const parts = [];
      ...
      if (el.chipGPU) {
        el.chipGPU.textContent = gpuAvail
          ? (gpuP == null ? "GPU: N/A" : `GPU: ${gpuP.toFixed(0)}%`)
          : "GPU: אין";
        setBadgeBg(el.chipGPU, gpuP);
      }
      ...
      if (el.envGPUFlags) {
        el.envGPUFlags.textContent = `NVML:${viaNVML ? "✓" : "×"} / CUDA:${viaCUDA ? "✓" : "×"}`;
      }
      if (el.envDocker) {
        if (isDocker) el.envDocker.classList.remove("hidden");
        else el.envDocker.classList.add("hidden");
      }

      // --- Full JSON (pretty) ---
      if (el.details) {
        el.details.textContent = JSON.stringify(data, null, 2);
      }
    } catch (e) {
