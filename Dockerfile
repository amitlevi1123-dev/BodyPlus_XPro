...
# רץ בענן (RunPod / כל Docker), עם Gunicorn ו-Healthcheck.
# לא מתקין constraints.txt.
# ============================================================
...
# ---- Healthcheck: קודם /healthz, אחרת /ping ----
HEALTHCHECK --interval=30s --timeout=5s --retries=5 \
  CMD curl -fsS "http://127.0.0.1:${PORT_HEALTH}/healthz" || curl -fsS "http://127.0.0.1:${PORT_HEALTH}/ping" || exit 1

# ---- הרצה (Shell form כדי להרחיב ${PORT}) ----
# חשוב: אל תכניס עברית/תווים לא-ASCII בשורות הפקודה.
